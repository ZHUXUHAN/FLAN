...
class RelationFeatureExtractor(nn.Module):
    ...
    def forward(self, x, proposals, rel_pair_idxs=None, proposals_part=None, part=False, object='obj'):
        if part:
            return self.forward_part(x, proposals, rel_pair_idxs, proposals_part, object=object)
        device = x[0].device
        union_proposals = []
        rect_inputs = []
    ...
    def forward_part(self, x, proposals, rel_pair_idxs=None, proposals_part=None, object='obj'):
        device = x[0].device
        union_proposals = []
        rect_inputs = []

        for proposal, rel_pair_idx, proposal_part in zip(proposals, rel_pair_idxs, proposals_part):
            if object == 'obj':
                head_proposal = proposal[rel_pair_idx[:, 0]]
                tail_proposal = proposal_part[rel_pair_idx[:, 1]]
            else:
                head_proposal = proposal_part[rel_pair_idx[:, 0]]
                tail_proposal = proposal[rel_pair_idx[:, 1]]
            union_proposal = boxlist_union(head_proposal, tail_proposal)
            union_proposals.append(union_proposal)
